"""Run the four ablation variants and emit a comparison table plus plots.

Variants:
  1. basic        no FPG message passing, no separation loss
  2. graph        FPG message passing, no separation loss
  3. sep          no FPG message passing, separation loss enabled
  4. graph_sep    full model: FPG message passing + separation loss
...
ABLATION_VARIANTS = [
    {"name": "basic",     "use_graph": False, "use_sep": False},
    {"name": "graph",     "use_graph": True,  "use_sep": False},
    {"name": "sep",       "use_graph": False, "use_sep": True},
    {"name": "graph_sep", "use_graph": True,  "use_sep": True},
]
...
def build_comparison_table(results, arch):
    ...
    for variant_name, summary in results.items():
        row = {
            ...
            "sep": summary.get("use_sep", False),
            "level1_auroc": summary["stage1_detection_auroc"]["mean"],
            "level1_f1": summary["stage1_detection_f1"]["mean"],
            "level2_cat_f1": summary["stage2_category_f1"]["mean"],
            "level3_rc_pred": summary["stage3_rc_pred_cat_macro"]["mean"],
        }
        rows.append(row)
    return rows
...
def print_table(rows, arch):
    ...
    print(f"{'='*90}")
    header = (f"{'Variant':<14} {'Graph':>5} {'Sep':>5} "
              f"{'L1-AUROC':>9} {'L1-F1':>7} "
              f"{'L2-CatF1':>9} {'L3-RC(pred)':>12}")
    print(header)
    print("-" * 92)
    for r in rows:
        g = "Y" if r["graph"] else "N"
        s = "Y" if r["sep"] else "N"
        print(f"{r['variant']:<14} {g:>5} {s:>5} "
              f"{r['level1_auroc']:>9.4f} {r['level1_f1']:>7.4f} "
              f"{r['level2_cat_f1']:>9.4f} {r['level3_rc_pred']:>12.4f}")
    print(f"{'='*90}")

    if len(rows) >= 4:
        basic = next((r for r in rows if r["variant"] == "basic"), None)
        graph = next((r for r in rows if r["variant"] == "graph"), None)
        sep = next((r for r in rows if r["variant"] == "sep"), None)
        full = next((r for r in rows if r["variant"] == "graph_sep"), None)

        if basic and graph:
            delta_cat = graph["level2_cat_f1"] - basic["level2_cat_f1"]
            print("\n  Q1: Does FPG message passing improve categorization?")
            print(f"      Graph vs Basic: {delta_cat:+.4f} Cat-F1")
            print(f"      -> {'YES' if delta_cat > 0.005 else 'MARGINAL' if delta_cat > 0 else 'NO'}")

        if basic and sep:
            delta_rc = sep["level3_rc_pred"] - basic["level3_rc_pred"]
            print("\n  Q2: Does the separation loss improve root-cause diagnosis?")
            print(f"      Sep vs Basic: {delta_rc:+.4f} RC-F1 (predicted category)")
            print(f"      -> {'YES' if delta_rc > 0.005 else 'MARGINAL' if delta_rc > 0 else 'NO'}")

        if basic and full:
            delta_all = full["level3_rc_pred"] - basic["level3_rc_pred"]
            print("\n  Q3: Does combining both give the best end-to-end RC(pred)?")
...
def generate_all_plots(results, arch, category_names, results_dir=None):
    ...
    proposed = "graph_sep" if "graph_sep" in fold0_data else list(fold0_data.keys())[-1]
...
def main():
    ...
    config = {
        ...
        **merged_training,
        ...
        "patience": int(merged_training.get("patience", 20)),
        "alpha": float(merged_training.get("alpha", 1.0)),
        "lambda_rc": float(merged_training.get("lambda_rc", 1.0)),
        "beta": float(merged_training.get("beta", 0.5)),
        "gamma": float(merged_training.get("gamma", 0.3)),
        "temperature": float(merged_training.get(
            "temperature", base.get("model", {}).get("temperature", 0.1))),
        "seed": 42,
    }
