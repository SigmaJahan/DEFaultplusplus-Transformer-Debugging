"""Within-family root cause diagnosis via signature matching.
...
Self-contained version for Compute Canada.
  python run_signature_matching.py --arch both
"""
...
RESULTS_DIR = REPO_ROOT / "results" / "diagnosis"
N_SPLITS = 5
RNG = 42
...
def run_signature_matching(arch_prefix):
    arch_name = "encoder" if arch_prefix == "enc" else "decoder"
    print(f"\n{'='*70}")
    print(f"  ROOT CAUSE DIAGNOSIS -- {arch_name.upper()}")
    print(f"{'='*70}")
...
def main():
    p = argparse.ArgumentParser(description="Root cause signature matching evaluation")
    ...
    for arch in archs:
        t0 = time.time()
        result = run_signature_matching(arch)
        out_path = RESULTS_DIR / f"{arch}_signature_matching.json"
