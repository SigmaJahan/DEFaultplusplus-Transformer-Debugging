"""Training script for hierarchical graph-aware fault diagnosis.

Trains the full 3-stage hierarchical model with 4-loss combination:
  L = L_detect + alpha * L_category + lambda * L_rootcause + beta * L_sibling

Training logic:
  - Detection uses ALL samples (clean + faulty), with class-weighted loss
  - Clean samples are oversampled within each batch to handle extreme imbalance
  - Category prediction uses FAULTY samples only
  - Root-cause prediction uses TRUE category (teacher forcing)
  - Intra-family contrastive loss computed within same-category samples

Key design choices:
  - Train/val split (80/20 stratified) WITHIN each CV fold for early stopping
  - Class-weighted detection loss (inverse frequency) + oversampling
  - RC(pred) evaluation counts misclassified-category samples as errors
  - Flat and graph encoders use matched capacity (same bottleneck width)

Ablation variants (controlled by flags):
  1. --no-graph --no-sibling   (basic hierarchical, no graph, no intra-family contrastive loss)
  2. --no-sibling              (graph added, no intra-family contrastive loss)
  3. --no-graph                (no graph, intra-family contrastive loss added)
  4. (default)                 (graph + intra-family contrastive loss -- full method)

Usage:
    python experiments/hierarchical_graph_category_rootcause/train.py --arch encoder
    python experiments/hierarchical_graph_category_rootcause/train.py --arch both
"""
import argparse
import json
import sys
...
DATA_DIR = _ROOT / "data"
ORIGIN_DIR = _ROOT / "data"
RESULTS_DIR = _ROOT / "results" / "hierarchical_graph_category_rootcause"
...
def train_one_fold(model, X_tr, y_det_tr, y_cat_tr, y_rc_tr,
    ...
    beta = config.get("beta", 0.5) if use_sibling else 0.0
    sib_temp = config.get("sibling_temperature", 0.1)
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, len(X_t_s), bs):
            ...
            loss, loss_dict = hierarchical_loss(
                ...
                alpha=alpha, lambda_=lambda_, beta=beta, gamma=gamma,
                sibling_temperature=sib_temp,
        ...
        if (epoch + 1) % 5 == 0:
            ...
            training_curves["loss_contrastive"].append(ep_losses["sibling"] / max(n_batches, 1))
...
def run_experiment(arch, use_graph=True, use_sibling=True, config=None):
    """Run full experiment for one architecture with specified ablation."""
    config = config or {}
    variant = []
    if use_graph:
        variant.append("graph")
    if use_sibling:
        variant.append("sibling")
    ...
    for fold_idx, (tr_idx, te_idx) in enumerate(gkf.split(X, y_detect, groups)):
        ...
        scaler, fold_curves = train_one_fold(
            ...
            config, use_sibling=use_sibling)
...
def main():
    ap = argparse.ArgumentParser(description="Hierarchical Graph-Aware Fault Diagnosis")
    ap.add_argument("--arch", choices=["encoder", "decoder", "both"], default="both")
    ap.add_argument("--no-graph", action="store_true", help="Disable FPG graph conditioning")
    ap.add_argument("--no-sibling", action="store_true", help="Disable intra-family contrastive loss")
    ...
    ap.add_argument("--lambda_", type=float, default=1.0, help="Root-cause loss weight")
    ap.add_argument("--beta", type=float, default=0.5, help="intra-family contrastive loss weight")
    args = ap.parse_args()

    import yaml
    config_path = Path(__file__).resolve().parent.parent.parent / "configs" / "base.yaml"
    with open(config_path) as f:
        base = yaml.safe_load(f)
    training_cfg = base.get("training", {}).copy()
    if "lambda_rootcause" in training_cfg and "lambda_" not in training_cfg:
        training_cfg["lambda_"] = training_cfg["lambda_rootcause"]
    if "beta_sibling" in training_cfg and "beta" not in training_cfg:
        training_cfg["beta"] = training_cfg["beta_sibling"]
    ...
    use_sibling = not args.no_sibling

    variant_parts = []
    if use_graph:
        variant_parts.append("graph")
    if use_sibling:
        variant_parts.append("sibling")
    ...
    for arch in archs:
        summary = run_experiment(arch, use_graph=use_graph,
                                 use_sibling=use_sibling, config=config)
    ...
    # Update status
    status_path = Path(__file__).resolve().parent.parent.parent / "results" / "status.json"
