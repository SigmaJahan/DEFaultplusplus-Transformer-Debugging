...
from realworld_evaluation.common import CheckResult, ContractEvaluation, IssueContract
...
CONTRACTS: dict[str, IssueContract] = {
    ...
    "issue_11903_diffusers_qkv_fusion": IssueContract(
        trigger="Fuse the q, k, v projections into to_qkv and attach LoRA to the original split projection modules.",
        mechanism="The buggy path updates the stale split projections while the forward pass reads only the fused to_qkv weight.",
        observable_symptom="Parameters change during fine-tuning but the model output does not move.",
    ),
}
...
def _mechanism_check(slug: str, details: dict[str, object]) -> CheckResult:
    ...
    if slug == "issue_11903_diffusers_qkv_fusion":
        stale = float(details["buggy_stale_update_ratio"])
        active = float(details["buggy_active_update_ratio"])
        return _check(
            stale > 1e-3 and active == 0.0,
            "the stale split projections are updated while the active fused path is not",
            "the stale-update signature is not present in the buggy path",
        )
    raise KeyError(slug)
...
def _symptom_check(slug: str, details: dict[str, object]) -> CheckResult:
    ...
    if slug == "issue_11903_diffusers_qkv_fusion":
        output_delta = float(details["buggy_output_delta"])
        return _check(
            output_delta < 1e-10,
            "updating the stale projections leaves the model output unchanged",
            "the buggy update unexpectedly moved the output",
        )
    raise KeyError(slug)
...
def _comparison_check(slug: str, details: dict[str, object]) -> CheckResult:
    ...
    if slug == "issue_11903_diffusers_qkv_fusion":
        buggy_delta = float(details["buggy_output_delta"])
        fixed_delta = float(details["fixed_output_delta"])
        return _check(
            buggy_delta < 1e-10 and fixed_delta > 1e-3,
            "targeting the fused path makes the update affect the output, unlike the stale-projection update",
            "buggy-vs-fixed update comparison is not decisive",
        )
    raise KeyError(slug)
