"""
JSON dataset exporter for ABNN Fault Injection runs.
...
from src.utils.config_generator import generate_all_configurations
...
class JSONDatasetBuilder:
    ...
    def build(self, output_path: str, indent: int = 2) -> List[Dict[str, Any]]:
        """Serialize all available configurations to JSON."""
    ...
    @staticmethod
    def _build_model_section(model_info: Dict[str, Any], training_cfg: Dict[str, Any]) -> Dict[str, Any]:
        name = model_info.get("model_name") or training_cfg.get("model_name") or "distilbert-base-uncased"
        num_layers = int(model_info.get("num_layers", training_cfg.get("num_hidden_layers", 6)))
        mapping = {
            f"L{idx}": f"distilbert.transformer.layer.{idx}"
    ...
    def _build_training_section(self, training_cfg: Dict[str, Any], run_metadata: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        section = {
    ...
    # ------------------------------------------------------------------ #
    def _build_labels_section(
        self,
        config_row: Dict[str, Any],
        kill_row: Optional[Dict[str, Any]],
    ) -> Dict[str, Any]:
    ...
    def _load_pipeline_configs(config_path: Optional[str]) -> Dict[str, Any]:
        if config_path and Path(config_path).exists():
            ...
            try:
                ...
                raise RuntimeError(
                    "PyYAML is required to read YAML pipeline configs. Install pyyaml or use JSON."
                ) from exc
            with open(path, "r") as handle:
                return yaml.safe_load(handle)
        return generate_all_configurations()
...
def main():
    parser = argparse.ArgumentParser(description="Export ABNN dataset to JSON")
    parser.add_argument("--master-config", default="config/master_config.yaml", help="Path to master_config.yaml")
    parser.add_argument("--h5", default="results/metrics.h5", help="Path to metrics HDF5 file")
    parser.add_argument("--db", default="results/dataset.db", help="Path to SQLite database with labels")
    parser.add_argument(
        "--pipeline-config",
        default="config/pipeline_configs.json",
        help="Optional pipeline config file (YAML or JSON)",
    )
    parser.add_argument("--output", default="results/final_dataset/dataset.json", help="Output JSON file")
