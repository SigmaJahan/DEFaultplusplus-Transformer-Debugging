"""
Metric collector for ABNN Fault Injection Dataset.

Handles per-batch metric computation, epoch-level aggregation with
Welford statistics, validation logging, and feature generation for
downstream analysis.
"""
...
class RepresentationDriftTracker:
    """
    Tracks hidden representations at selected epochs for cosine drift and CKA.
    Stores sample matrices for linear CKA and mean vectors for cosine drift.
    """
    ...
    def _linear_cka(X: torch.Tensor, Y: torch.Tensor) -> float:
        ...
        X, Y: [n_samples, features] (mean-centered).
        """
    ...
    def get_features(self, num_layers: int) -> Dict[str, float]:
        """Return cosine drift and CKA between reference epochs."""
        ...
        # Free sample memory after feature extraction
...
def _safe_compute(func, *args, label: str = "", **kwargs) -> Dict[str, float]:
    """Call a metric function, returning empty dict on failure."""
    try:
...
class MetricCollector:
    """
    Collects, aggregates, and summarizes metrics during training.
    """
    ...
    def __init__(
        ...
        self.num_layers = self.config.get('num_hidden_layers', self.config.get('model_config', {}).get('num_hidden_layers', 6))
    ...
    def collect_batch_metrics(
        ...
        """
        Collect metrics for a single (possibly accumulated) training step.
        """
        ...
        # Attention metrics on sampled layers
        ...
        if self.collect_attention and attentions is not None:
            ...
            for layer_idx in sample_layers:
                ...
                if 'attention_mass_pad_mean' in attn_metrics:
                    attention_alias_accumulator['mass_pad'].append(attn_metrics['attention_mass_pad_mean'])
                if 'attention_mass_leak' in attn_metrics:
                    attention_alias_accumulator['mass_leak'].append(attn_metrics['attention_mass_leak'])
                if 'attention_cross_example_leak' in attn_metrics:
                    attention_alias_accumulator['cross_example_attention'].append(attn_metrics['attention_cross_example_leak'])
                if 'attention_mass_future' in attn_metrics:
                    attention_alias_accumulator['attention_mass_future'].append(attn_metrics['attention_mass_future'])
                if 'attention_entropy' in attn_metrics:
                    attention_alias_accumulator['attention_entropy'].append(attn_metrics['attention_entropy'])
                if 'pre_softmax_score_mean' in attn_metrics:
                    attention_alias_accumulator['pre_softmax_score_mean'].append(attn_metrics['pre_softmax_score_mean'])
                if 'pre_softmax_score_var' in attn_metrics:
                    attention_alias_accumulator['pre_softmax_score_var'].append(attn_metrics['pre_softmax_score_var'])
                if 'pre_softmax_score_skew' in attn_metrics:
                    attention_alias_accumulator['pre_softmax_score_skew'].append(attn_metrics['pre_softmax_score_skew'])
                if 'pre_softmax_score_kurt' in attn_metrics:
                    attention_alias_accumulator['pre_softmax_score_kurt'].append(attn_metrics['pre_softmax_score_kurt'])
                if 'head_similarity_mean' in attn_metrics:
                    attention_alias_accumulator['head_similarity_mean'].append(attn_metrics['head_similarity_mean'])
                if 'head_similarity_max' in attn_metrics:
                    attention_alias_accumulator['head_similarity_max'].append(attn_metrics['head_similarity_max'])
                if 'attention_rank_mean' in attn_metrics:
                    attention_alias_accumulator['attention_rank_mean'].append(attn_metrics['attention_rank_mean'])
    ...
    def finalize_epoch(self, epoch: int) -> Dict[str, float]:
        """Finalize the running statistics for the epoch."""
    ...
    def record_validation_metrics(self, epoch: int, metrics: Dict[str, float]):
        """Store validation metrics and extend history for windowed features."""
    ...
    def get_validation_history(self) -> List[Dict[str, float]]:
        """Return validation history snapshots."""
    ...
    def _aggregate_layer_metrics(
        ...
        """Paper A_ℓ=5: aggregate per-layer metrics into layer-position groups.

        For each per-layer metric (keyed as L{i}_{name}_mean or {name}_l{i}_mean),
        produces 5 summary stats:
          {name}_layer_early_mean, {name}_layer_early_std,
          {name}_layer_mid_mean,   {name}_layer_mid_std,
          {name}_layer_final
        """
        import re
        import numpy as np
        ...
        # Split layers into 3 groups as evenly as possible
        base = num_layers // 3
    ...
    def get_final_metrics(self) -> Dict[str, float]:
        ...
        # Paper A_ℓ=5: layer-position aggregation (early/mid/final layer groups)
        final_metrics.update(self._aggregate_layer_metrics(last_epoch, self.num_layers))

        # Representation drift features
        ...
        if self.validation_history:
            ...
            # CRITICAL FIX (Bug #17b): Only set val metrics if they exist
            # For LM tasks, val_accuracy doesn't exist, so don't set it to 0.0
            # This allows fallback to train metrics to work correctly
            ...
            # Copy generation metrics (already properly named)
            for key in ('val_repetition_max_run', 'val_repetition_distinct_1', 'val_repetition_distinct_2',
                        'val_generation_mean_length', 'val_generation_eos_ratio',
                        'val_cache_correctness', 'val_cache_nll_divergence'):
                if key in last_val:
                    final_metrics[key[4:]] = last_val[key]  # strip val_ prefix
        ...
        # Backwards-compatible aliases - prefer validation metrics if available, fallback to train
        # For decoder LM tasks, use loss/perplexity as primary metrics
    ...
    def _summarize_attention_aliases(alias_values: Dict[str, List[float]]) -> Dict[str, float]:
        """Map layer-specific attention stats to kill-function aliases."""
        ...
        summary['attention_mass_future'] = max(alias_values['attention_mass_future']) if alias_values['attention_mass_future'] else 0.0
