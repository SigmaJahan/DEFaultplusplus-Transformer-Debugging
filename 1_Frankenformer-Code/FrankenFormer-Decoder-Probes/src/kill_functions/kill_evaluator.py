"""
Kill-function evaluation runner.

Loads metrics/labels from results artifacts, aligns clean vs faulty runs
on the available epochs, and executes the kill criteria to compute
p-values and decisions. Results are persisted back into dataset.db.
"""
...
# Layer prefixes for per-head similarity metrics; add a few layers to be robust.
...
CANO_METRIC_KEYS: Dict[str, List[str]] = {
    ...
    # Decoder eval metrics (canonicalized from val_ prefix after dedup)
    "eval_loss": ["final_eval_loss", "eval_loss_final", "val_loss_final", "final_val_loss"],
    "eval_perplexity": ["final_eval_perplexity", "eval_perplexity_final", "val_perplexity_final", "final_val_perplexity"],
    ...
    "cache_correctness": ["cache_correctness_final", "val_cache_correctness_final", "metric_25_final", "final_metric_25"],
    "cache_nll_divergence": ["cache_nll_divergence_final", "val_cache_nll_divergence_final", "metric_26_final", "final_metric_26"],
    ...
    "head_similarity_mean": [
        *[
            f"{prefix}head_similarity_mean_final"
            for prefix in HEAD_SIM_PREFIXES
        ],
        *[
            f"{prefix}head_similarity_mean_late_mean"
            for prefix in HEAD_SIM_PREFIXES
        ],
        *[
            f"{prefix}head_similarity_mean_mid_mean"
            for prefix in HEAD_SIM_PREFIXES
        ],
        *[
            f"{prefix}head_similarity_mean_early_mean"
            for prefix in HEAD_SIM_PREFIXES
        ],
    ],
    "embedding_norm_mean": [
...
class MetricsSnapshot:
    ...
    def build_final_metrics(self, max_epochs: Optional[int] = None) -> Dict[str, float]:
        ...
        # Merge in stored final_metrics so specialized features (e.g., embedding/logit stats)
        # survive the kill-eval canonicalization step.
        merged = dict(rebuilt)
...
class KillEvaluationRunner:
    ...
    def _align_series(
        ...
        We deliberately avoid fabricating defaults; metrics absent on either side
        remain unavailable so the validator can mark them as such.
        """
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--results-dir", default="results", help="Directory that contains metrics.h5 and dataset.db")
    parser.add_argument(
        "--allowed-seeds",
        type=str,
        default=None,
...
def main() -> None:
    ...
    runner = KillEvaluationRunner(results_dir=Path(args.results_dir), allowed_seeds=allowed_seeds, alpha=args.alpha)
    runner.evaluate()
