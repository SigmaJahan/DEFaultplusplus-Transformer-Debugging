#!/bin/bash
# FrankenFormer-Decoder fully automated environment setup
...
#
# First time setup:
#   Edit scripts/env_config.sh and customize paths for your cluster if needed
...
echo "========================================="
echo "FrankenFormer-Decoder Automated Setup"
...
# Install datasets separately without dependencies to avoid triggering CC's pyarrow dummy wheel upgrade
...
echo "========================================="
"$PYTHON_BIN" "$SCRIPT_DIR/cache-models-datasets.py" --matrix-config "$PROJECT_ROOT/config/matrix_336.yaml"

echo "========================================="
echo "Running Dry-Run Pipeline Check..."
echo "========================================="
"$PYTHON_BIN" "$SCRIPT_DIR/run_pipeline.py" \
    --matrix-config "$PROJECT_ROOT/config/matrix_336.yaml" \
    --config-file "$PROJECT_ROOT/config/pipeline_configs_probes.json" \
...
echo "  sbatch scripts/submit_probes_336.sh"
echo "========================================="