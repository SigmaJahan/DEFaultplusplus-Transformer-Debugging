#!/bin/bash
# Encoder structural-probe run: stratified fault configurations
# 6 (model, dataset) pairs x 56 configs each = 336 experiments
#
# Chunked into 18 array tasks (3 chunks per pair, 20 configs each).
# Encoder models are smaller than decoders, so expect faster runtimes.
#
# Usage (from project root):
#   sbatch scripts/submit_encoder_probes.sh
#
#SBATCH --job-name=enc-probes
...
MODELS=("distilbert-base-uncased" "distilbert-base-uncased" "bert-base-uncased" "bert-base-uncased" "roberta-base" "roberta-base")
DATASETS=("sst2" "mnli" "sst2" "mnli" "sst2" "mnli")
...
if [ -f "$VENV_TARBALL" ] && [ -n "${SLURM_TMPDIR:-}" ]; then
    ...
    PYTHON_BIN="/project/def-mrdal22/sjahan/venv-encoder/bin/python"
...
MATRIX_CONFIG="$PROJECT_ROOT/config/matrix_encoder.yaml"
...
echo "================================================================================"
echo "Encoder Structural Probe Run"
echo "================================================================================"
...
$PYTHON_BIN scripts/run_pipeline.py \
    ...
    --fault-config "$PIPELINE_CONFIG" \
    --results-dir "$RESULTS_DIR" \
    --cuda \
    --config-index "$START_INDEX"
